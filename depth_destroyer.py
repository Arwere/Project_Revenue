...
class DepthDestroyer:
    ...
    async def tick(self):
        try:
            ...
            decision = self.agent.get_risk_adjusted_decision(
                self.config, {"price_sol": current_price}, prices, bot_name="DepthDestroyer"
            )
    ...
    async def _execute_buy(self, price: float, decision: dict):
        amount_sol = self.wallet.get_available_sol() * decision.get("suggested_capital_percent", 0.30)
        if amount_sol < 0.05:
            return
        print(f"[DEPTH DESTROYER] Executing BUY {amount_sol:.4f} SOL → {self.config.symbol}")
        tx = await self.jupiter.swap_sol_to_token(self.config.address, amount_sol, slippage=1.8)
        if tx:
            self.position = (amount_sol / price) * 0.98
            self.entry_price = price
            print(f"[DEPTH DESTROYER] BUY SUCCESS")

    async def _execute_sell(self, price: float, decision: dict):
        if self.position <= 0:
            return
        print(f"[DEPTH DESTROYER] Executing SELL")
        tx = await self.jupiter.swap_token_to_sol(self.config.address, self.position, slippage=1.8)
        if tx:
            self.position = 0.0
            self.cooldown_until = time.time() + 240
            print(f"[DEPTH DESTROYER] SELL SUCCESS")

if __name__ == "__main__":
    print("Depth Destroyer initialized")
EOF
